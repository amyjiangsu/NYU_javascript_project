...
var treeWidth;
var shortHeight;
var nextImage = countDown + ".png";


var clock = svg.selectAll("image").data([0])
  .enter()
  .append("svg:image")
...
svg.append("svg:image")
  ...
  .on("mousedown", function() {
	if(countDown == 0) {
	  setEventHandler('', 'mousedown');
	  svg.selectAll("image").style("opacity",0);
	  d3.select("body").style("background","#4e7019");
	  if (width >= height*1.2) {
	  	yearWidth = height*0.45;
	  	treeWidth = height*1.05;
	  } else {
	  	yearWidth = width/1.1 * 0.4;
	  	treeWidth = width/1.1;
	  }
	  
	  svg.append("svg:image")
	  .attr("xlink:href","2015.png")
	  .attr("x", (width-treeWidth*1.1)/2)
	  .attr("y", 0)
