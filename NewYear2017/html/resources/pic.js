...
*
* For me on GitHub:  https://github.com/vogievetsky/picsToTheMax
* License: MIT  [ http://picstothemax.com/LICENSE ]
*
* If you are reading this then I have an easter egg for you:
* You can use your own custom image as the source, simply type in:
* http://picstothemax.com?<your image url>
* e.g.
* http://picstothemax.com?http://upload.wikimedia.org/wikipedia/commons/thumb/a/ae/Flag_of_the_United_Kingdom.svg/200px-Flag_of_the_United_Kingdom.svg.png
*
...
* http://picstothemax.com?<your image url in base64>
* e.g.
* http://picstothemax.com?YXN0bGV5LmpwZw==
...
(function() {
  ...
  pic.supportsCanvas = function() {
    var elem = document.createElement('canvas');
    return !!(elem.getContext && elem.getContext('2d'));
  };

  pic.supportsSVG = function() {
  ...
  Circle.addToVis = function(vis, circles, init) {
    var circle = vis.selectAll('.nope').data(circles)
      .enter().append('circle');
	var title = vis.selectAll('.nope').data(circles)
		  .enter().append('text');
	var caption = vis.selectAll('.nope').data(circles)
		  .enter().append('text');
	var remark = vis.selectAll('.nope').data(circles)
		  .enter().append('text');
    ...
    if (init) {
      ...
      circle = circle
		...
		circle
		  .attr('cx',   function(d) { return d.x; })
		  .attr('cy',   function(d) { return d.y/2 + 30; })
		...
		.attr("dy", function(d){return d.y/2 - 30})
        .text("Happy New Year!")
	   ...
	   remark
        .attr("dx", function(d){return d.x})
		.attr("dy", function(d){return d.y/2 + 120})
        .html("(This is a scratch card, please keep scratching the screen...)")
		.attr("text-anchor", "middle")
  ...
  pic.loadImage = function(imageData) {
  ...
  pic.makeCircles = function(selector, colorData, onEvent) {
    ...
    d3.select(document.body)
      .on('mousemove.pic', onMouseMove)
      .on('touchmove.pic', onTouchMove)
      .on('touchend.pic', onTouchEnd)
      .on('touchcancel.pic', onTouchEnd);
  };
})();
